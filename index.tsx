...
const getImgPath = (name: string) => {
  // Removes titles like "Prof." and converts to "firstname-lastname.jpg"
...
const TEAM_DATA = {
  ...
  alumni: [
    { name: "Yulia Gryaditskaya", current: "Senior Research Scientist at Adobe", image: "yulia-gryaditskaya.jpg" },
...
const SketchingBrain = ({ className }: { className?: string }) => (
  <svg 
...
const MemberAvatar = ({ name, image, className = "" }: { name: string, image?: string, className?: string }) => {
  const [error, setError] = useState(false);
        ...
        <div className="hidden md:flex gap-8">
          {links.map((link) => (
            <button
              ...
              className={`text-sm font-medium hover:text-orange-600 transition-colors ${activeSection === link.id ? 'text-black border-b-2 border-orange-600' : 'text-gray-500'}`}
            >
              {link.label}
            </button>
          ))}
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 p-6 flex flex-col gap-4 shadow-lg">
          {links.map((link) => (
            <button
              ...
              className="text-left text-lg font-medium text-gray-800 hover:text-orange-600"
            >
              {link.label}
            </button>
          ))}
      ...
      <div className="max-w-7xl mx-auto px-6 text-center z-10 pt-10">
        ...
        <div className="w-full mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-md transition-shadow">
            ...
            <p className="text-gray-500 text-sm">Decoding semantics by leveraging the nuance and abstraction that sketches possess and text lacks.</p>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-md transition-shadow">
          ...
          {NEW_PUBLICATIONS.map((pub, idx) => (
            <div key={idx} className="group bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full hover:border-orange-200 relative">
              <div className="h-48 bg-white overflow-hidden relative">
                <img src={pub.image} alt={pub.title} className="w-full h-full object-fill transition-transform duration-500 group-hover:scale-105 opacity-90 group-hover:opacity-100" />
